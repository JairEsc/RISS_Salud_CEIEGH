...
demograficos_scince=rbind(ageb_pob |> dplyr::mutate(NOM_MUN=NA,NOMGEO=NA,CVE_AGEB=NA),loc_rural_pe|> dplyr::select(CVEGEO,NOM_MUN,NOMGEO,CVE_AGEB,POB1,POB42,POB84, dplyr::all_of(paste0("POB",17:25)),, dplyr::all_of(paste0("SALUD",1:10))  
) |> st_buffer(150)) 
demograficos_scince |> st_simplify(preserveTopology = T,dTolerance = 20) |> st_write("outputs/demograficos_cartograficos_scince_20.geojson",driver='GeoJSON')
