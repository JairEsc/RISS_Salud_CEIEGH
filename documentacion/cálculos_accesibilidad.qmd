...
lista_simulaciones=list(simular_rutas(0),
                         simular_rutas(10),
                         simular_rutas(1000)
                        )
lista_plots=list()
...
Lo anterior se repite para cada CLUES lo cual define una matriz de costos de dimensión \$\|C_k\|\\times \|puntos(AGEB)\| \$. De esta matriz podemos extraer información valiosa que corresponde a:
...
tiempo_zona_individual_croped  |> plot()
plot(A$geometry |> st_transform(32614) ,add=T)
#+hist
A_extract=exactextractr::exact_extract(tiempo_zona_individual_croped,A |> st_transform(32614),'frac' )
plot(gsub("frac_","",colnames(A_extract)) |> as.numeric(),A_extract[1,],"l")
#Pendiente hacerlo en bonito
```

##### pendiente. Dibujito de la cobertura de un polígono coloreado con el costo de traslado a un CLUES
