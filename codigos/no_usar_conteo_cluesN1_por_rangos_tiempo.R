...
clues_solicitados=clues_solicitados |> #Filtramos los que se salen del estado
  st_intersection(limites_municipales |> dplyr::select(geometry) |> st_transform(4326)) 

tiempo_zona=accCost(T.GC, matrix(unlist(clues_solicitados$geometry |> 
                                          st_transform(32614)),
                                 nrow = nrow(clues_solicitados),
                                 ncol = 2,byrow = T))
...
for(cve_unica in unique(resultado$CVEGEO)){
  ...
  conteo_clues_N1_por_ageb[nrow(conteo_clues_N1_por_ageb)+1,]=c(
    cve_unica,num_clues_a_menos_de_10,
    num_clues_a_menos_de_20,
    num_clues_a_menos_de_40,
